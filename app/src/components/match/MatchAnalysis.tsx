...
export const MATCH_SYSTEM_PROMPT = `# Role
...
export function MatchAnalysis() {
  ...
  useEffect(() => {
    if (result && matchChatHistory.length === 0) {
      ...
      const initialUserMessage = `请分析以下两人的命盘契合度：
...
请分析两人的契合度和相处建议。`
      ...
      try {
        await fetch('/api/user/history', {
          ...
          body: JSON.stringify({
            type: 'match',
            title: `双人合盘 - ${person1.gender === 'male' ? '男' : '女'}${person1.year}年 & ${person2.gender === 'male' ? '男' : '女'}${person2.year}年`,
            content: fullText,
            birth_info: { person1, person2 },
          }),
        })
      } catch {}
    } catch (err) {
        ...
        {result && (
          <>
            ...
            <FollowUpQuestion
              ...
              chatHistory={matchChatHistory}
              onChatHistoryChange={setMatchChatHistory}
              llmConfig={llmConfig}
              placeholder="基于以上合盘解读，有什么问题想问吗？"
            />
          </>
        )}
      </div>
