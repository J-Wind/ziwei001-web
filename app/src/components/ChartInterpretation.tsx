...
const MarkdownComponents = {
  h1: ({ children }: { children?: React.ReactNode }) => (
    <h1 className="text-2xl font-bold mt-6 mb-3 first:mt-0 text-gold">{children}</h1>
  ),
  h2: ({ children }: { children?: React.ReactNode }) => (
    <h2 className="text-xl font-semibold mt-5 mb-2 text-star-dark">{children}</h2>
  ),
  h3: ({ children }: { children?: React.ReactNode }) => (
    <h3 className="text-lg font-medium mt-4 mb-2 text-star-dark/90">{children}</h3>
  ),
  p: ({ children }: { children?: React.ReactNode }) => (
    <p className="mb-3 leading-relaxed text-text/95">{children}</p>
  ),
  ...
  li: ({ children }: { children?: React.ReactNode }) => (
    <li className="relative pl-5 xs:pl-6 mb-2 leading-relaxed">
      <span className="absolute left-0 top-[0.35em] text-gold/70 text-xs select-none">◆</span>
      <span>{children}</span>
    </li>
  ),
  blockquote: ({ children }: { children?: React.ReactNode }) => (
    <blockquote className="border-l-2 border-gold/50 pl-4 my-3 italic text-text-secondary bg-gradient-to-r from-gold/8 to-transparent rounded-r-lg py-2">
      {children}
    </blockquote>
  ),
  code: ({ children, className }: { children?: React.ReactNode; className?: string }) => (
    className ? (
      ...
      <code className="px-1.5 py-0.5 rounded bg-gradient-to-r from-gold/15 to-gold/10 text-gold text-sm font-medium border border-gold/20">{children}</code>
    )
  ),
...
请结合当前年份${currentYear}年，给出详细但通俗易懂的命盘解读，确保所有年份相关的分析都基于${currentYear}年。`
      ...
      let errorMessage = '解读失败，请重试'

      if (err instanceof Error) {
        const message = err.message.toLowerCase()

        if (message.includes('401') || message.includes('unauthorized') || message.includes('未授权')) {
          errorMessage = '请先登录后再使用解读功能'
        } else if (message.includes('402') || message.includes('积分不足')) {
          errorMessage = '积分不足，请充值后继续使用'
        } else if (message.includes('api key') || message.includes('api_key') || message.includes('not configured')) {
          errorMessage = 'AI 服务暂时不可用，请联系管理员配置 API Key'
        } else if (message.includes('network') || message.includes('fetch') || message.includes('abort')) {
          errorMessage = '网络连接失败，请检查网络后重试'
        } else if (message.includes('timeout')) {
          errorMessage = '请求超时，请稍后重试'
        } else {
          errorMessage = err.message || '解读失败，请重试'
        }
      }
