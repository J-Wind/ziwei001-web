...
export const api = {
  ...
  recharge: {
    ...
    // Zpay 在线支付
    zpayCreate: (data: { amount: number; points: number; type?: string }) =>
      request<{ success: boolean; orderNo: string; qrcode?: string; img?: string; payUrl?: string; trade_no?: string; message: string }>('/api/recharge/zpay/create', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    zpayQuery: (orderNo: string) =>
      request<{ order_no: string; status: string; amount: number; points: number; created_at: string }>(`/api/recharge/zpay/query/${orderNo}`),
