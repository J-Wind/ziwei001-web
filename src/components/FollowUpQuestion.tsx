...
const MarkdownComponents = {
  h1: ({ children }: { children?: React.ReactNode }) => (
    <h1 className="text-xl font-bold mt-4 mb-2 first:mt-0 text-gold">{children}</h1>
  ),
  h2: ({ children }: { children?: React.ReactNode }) => (
    <h2 className="text-lg font-semibold mt-3 mb-1 text-star-dark">{children}</h2>
  ),
  h3: ({ children }: { children?: React.ReactNode }) => (
    <h3 className="text-md font-medium mt-2 mb-1 text-star-dark/90">{children}</h3>
  ),
  p: ({ children }: { children?: React.ReactNode }) => (
    <p className="mb-2 leading-relaxed text-text/95">{children}</p>
  ),
  ...
  li: ({ children }: { children?: React.ReactNode }) => (
    <li className="relative pl-5 xs:pl-6 mb-2 leading-relaxed">
      <span className="absolute left-0 top-[0.35em] text-gold/80 text-xs select-none">◆</span>
      <span>{children}</span>
    </li>
  ),
  blockquote: ({ children }: { children?: React.ReactNode }) => (
    <blockquote className="border-l-2 border-gold/60 pl-3 my-2 italic text-text-secondary bg-gradient-to-r from-gold/10 to-transparent rounded-r-lg py-2">
      {children}
    </blockquote>
  ),
  code: ({ children, className }: { children?: React.ReactNode; className?: string }) => (
    className ? (
      ...
      <code className="px-1.5 py-0.5 rounded bg-gradient-to-r from-gold/20 to-gold/15 text-gold text-sm font-medium border border-gold/30">{children}</code>
    )
  ),
  a: ({ children, href }: { children?: React.ReactNode; href?: string }) => (
    <a
      href={href}
      className="text-star-light hover:text-gold underline decoration-gold/30 hover:decoration-gold/60 transition-all duration-300"
      target="_blank"
...
export function FollowUpQuestion({
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 【关键修复】先立即将用户问题添加到历史记录，让UI立即显示问题
      const historyWithUserMsg = [...chatHistory, newMessage]
      onChatHistoryChange(historyWithUserMsg)

      // 准备发送给API的消息
      const messages: ChatMessage[] = [
        { role: 'system', content: systemPrompt },
        ...historyWithUserMsg.filter(msg => msg.role !== 'system'),
      ]
      ...
      // 完成后将AI回答也添加到历史记录
      const finalHistory = [...historyWithUserMsg, { role: 'assistant', content: fullAnswer }]
      ...
      if (err instanceof Error) {
        ...
        if (message.includes('401') || message.includes('unauthorized') || message.includes('未授权')) {
          ...
          errorMessage = '积分不足，无法继续提问'
        } else if (message.includes('api key') || message.includes('not configured')) {
          errorMessage = 'AI 服务暂时不可用'
        } else if (message.includes('network') || message.includes('fetch') || message.includes('abort')) {
