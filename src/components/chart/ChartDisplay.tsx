...
function StarTag({ star, showBrightness = true }: StarTagProps) {
  ...
  return (
    <span
      className={`
        inline-flex items-center gap-0.5 text-[9px] xs:text-[10px] sm:text-[11px] md:text-[13px] px-0.5 sm:px-1 py-0.5 rounded
    ...
    >
      ...
      {showBrightness && brightnessChar && (
        <span className={`text-[7px] xs:text-[8px] sm:text-[9px] md:text-[11px] ${brightnessStyle}`}>{brightnessChar}</span>
      )}
      {mutagen && <span className="text-[7px] xs:text-[8px] sm:text-[9px] md:text-[11px]">{mutagen}</span>}
    </span>
  )
}
...
function PalaceCard({
  ...
  return (
    <div
        onClick={onClick}
        className={`
          group relative p-0.5 xs:p-1 sm:p-1.5 md:p-2 lg:p-3 h-full flex flex-col
          bg-night/40 border border-white/[0.1] rounded
          transition-all duration-300 cursor-pointer
          hover:bg-night/60 hover:border-white/[0.2]
          ${isLife ? 'ring-1 ring-star border-gold/60 bg-gradient-to-br from-white/[0.06] to-white/[0.04]' : ''}
          ${isBody ? 'ring-1 ring-star/50 bg-star/[0.03]' : ''}
          ${isSelected ? 'ring-1.5 ring-star' : ''}
        `}
      >
        {/* 命宫金色光晕呼吸效果 */}
        {isLife && (
          <div className="absolute inset-0 rounded-md sm:rounded-xl pointer-events-none">
            <div className="absolute inset-0 rounded-md sm:rounded-xl shadow-[0_0_20px_rgba(212,175,55,0.4)]" />
          </div>
        )}

        {/* 宫位头部: 宫干支 + 宫名 + 大限 */}
        <div className={`flex items-center justify-between mb-0.5 xs:mb-1 text-[9px] xs:text-[10px] sm:text-[11px] md:text-[12px] ${isLife ? 'text-white' : ''}`}>
          <span className={`font-mono ${isLife ? 'text-gold/80' : 'text-text-muted'}`}>{stem}{branch}</span>
          <div className="flex items-center gap-0.5">
            {decadalRange && (
              <span className={`font-mono ${isLife ? 'text-gold/70' : 'text-star-light/60'}`}>{decadalRange}</span>
            )}
            <span className={`
              px-0.5 sm:px-1 py-0.5 rounded font-medium text-[9px] xs:text-[10px] sm:text-[11px] md:text-[12px]
              ${isLife ? 'bg-gold/30 text-gold font-bold' : ''}
        ...
        {/* 主星 */}
        <div className="flex flex-wrap gap-0.5 mb-0.5 xs:mb-1">
          {majorStars.map((star, i) => (
            <div key={i} className={isLife ? 'scale-[1.02]' : ''}>
              <StarTag star={star} />
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-0.5 mb-0.5 xs:mb-1">
          {minorStars.map((star, i) => (
            <div key={i} className={isLife ? 'scale-[1.02]' : ''}>
              <StarTag star={star} showBrightness={false} />
            </div>
          ))}
        </div>
        ...
        {adjectiveStars.length > 0 && (
          <div className="flex flex-wrap gap-0.5 mb-0.5 xs:mb-1 flex-1">
            {adjectiveStars.map((name, i) => (
              <span key={i} className={`text-[9px] xs:text-[10px] sm:text-[11px] md:text-[12px] px-0.5 sm:px-1 py-0.5 rounded ${isLife ? 'bg-gold/10 text-gold/80' : 'bg-white/[0.03] text-text-muted/70'}`}>
                {name}
              </span>
            ))}
          </div>
        ...
        {/* 底部: 十二神 */}
        <div className={`flex justify-between text-[9px] xs:text-[10px] sm:text-[11px] md:text-[12px] mt-auto pt-0.5 xs:pt-1 sm:pt-1.5 border-t ${isLife ? 'border-gold/20 text-gold/60' : 'border-white/[0.04] text-text-muted/60'}`}>
...
function CenterInfo({ chart, solarDate, gender }: CenterInfoProps) {
  ...
  return (
    <div className="
      relative h-full min-h-[200px] xs:min-h-[240px] sm:min-h-[260px] md:min-h-[280px] lg:min-h-[360px] p-2 xs:p-3 sm:p-3 lg:p-4
      flex flex-col items-center justify-center
      bg-night/60 backdrop-blur-md border border-white/[0.1] rounded-lg sm:rounded-xl
    ">
      {/* 背景装饰 */}
      <div className="absolute inset-0 opacity-[0.02]">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 xs:w-24 sm:w-28 h-20 xs:h-24 sm:h-28 rounded-full border-2 border-white" />
      </div>

      {/* 标题 */}
      <h3 className="
        text-[12px] xs:text-[14px] sm:text-lg lg:text-xl font-semibold mb-1 xs:mb-1.5 sm:mb-3
        text-gold
      ...
      {/* 信息列表 */}
      <div className="text-[9px] xs:text-[10px] sm:text-xs lg:text-sm text-text-secondary space-y-0.5 xs:space-y-1 sm:space-y-1.5 text-center">
      ...
      {/* 五行局 + 命主身主 */}
      <div className="mt-1 xs:mt-1.5 sm:mt-3 pt-1 xs:pt-1.5 sm:pt-3 border-t border-white/[0.06] w-full">
        <div className="flex justify-center gap-1 xs:gap-1.5 sm:gap-2 mb-0.5 xs:mb-1 sm:mb-2">
          <span className="
        ...
        <div className="flex justify-center gap-1.5 xs:gap-2 sm:gap-4 text-[9px] xs:text-[10px] sm:text-xs">
        ...
        <div className="flex justify-center gap-1.5 xs:gap-2 sm:gap-4 text-[9px] xs:text-[10px] sm:text-xs mt-0.5 xs:mt-1">
...
export function ChartDisplay() {
  ...
  return (
    <div className="
      relative p-2 xs:p-3 sm:p-3 lg:p-6
    ...
    ">
      ...
      {/* 4x4 网格 */}
      <div className="grid grid-cols-4 gap-2 xs:gap-2.5 sm:gap-2 lg:gap-2">
        {/* Row 0 */}
      ...
      {/* 图例 */}
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 mt-1.5 sm:mt-3 pt-1.5 sm:pt-3 border-t border-white/[0.06] text-[8px] sm:text-[10px]">
        <div className="flex items-center gap-0.5">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-gold" />
          <span className="text-text-muted">命宫</span>
        </div>
        <div className="flex items-center gap-0.5">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-star-light" />
